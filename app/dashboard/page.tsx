...
interface Assignment {
  ...
  paper: { id: string; slug?: string | null; title: string; authors: string };
}
...
/**
 * The signed-in home page, at /dashboard.
 *
 * "/" is the public landing page and nothing else; anyone signed in who lands
 * there is sent here, so a shared or bookmarked link never shows a researcher
 * the marketing page.
 */
        ...
        if (res.ok) {
          ...
          if (isStudent) {
            ...
            const assignments = assignmentsRes.ok
              ...
              : [];
            const notificationsPayload = notificationsRes.ok
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 glass-card border-border-default/80">
        <div className="space-y-1">
          <div className="flex items-center gap-2.5">
            <span
              className={`px-2 py-0.5 text-[11px] font-semibold tracking-wide uppercase rounded-md border ${roleBadgeColor}`}
            >
              {user?.systemRole || "STUDENT"}
            </span>
            <span className="text-xs text-text-tertiary">
              {user?.department ? `${user.department} · ` : ""}
              {user?.institution || "Academic Lab"}
            </span>
          </div>
          ...
          <p className="text-xs text-text-secondary">
            {roleTitle} — Review literature, track reading goals, and annotate
            contributions.
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-bold text-text-primary flex items-center gap-2 font-display">
              <Users size={18} className="text-purple-500" /> Supervised
              Students
            </h3>
            <Link
              href="/students"
          ...
          {stats.supervisedStudents && stats.supervisedStudents.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {stats.supervisedStudents.map((s) => (
                <Link
                  key={s.id}
                  href={`/papers?studentId=${s.id}`}
                  className="glass-card p-4 hover:border-purple-500/40 transition-all block group"
                >
            ...
            <div className="glass-card p-6 text-center text-xs text-text-tertiary">
              No students assigned yet. Contact your administrator or view the
              student directory.
            </div>
          )}
        </div>
      ...
      {isStudent && (
        <div className="space-y-6">
          ...
          <SupervisionRequestBanner
            onRequestResolved={() => window.location.reload()}
          />
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-text-primary flex items-center gap-2 font-display">
                <GraduationCap size={18} className="text-purple-400" /> Research
                &amp; Lab Performance
              </h3>
              <span className="text-xs text-text-tertiary">
                Synced with Advisor:{" "}
                <strong>{stats.supervisor?.name || "Faculty Mentor"}</strong>
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5">
              ...
              <Link
                href="/assignments"
                className="p-4 rounded-2xl glass-card space-y-2 hover:border-purple-500/40 transition-all block group"
              >
                <div className="flex items-center justify-between text-xs">
                  <span className="font-semibold text-text-primary flex items-center gap-1">
                    <BookOpen size={13} className="text-purple-400" /> Assigned
                    Reading
                  </span>
                  <span className="font-mono text-purple-400 font-bold">
                    {stats.assignedCompletionRate || 0}%
                  </span>
                </div>
                <div className="w-full bg-bg-primary h-2 rounded-full overflow-hidden">
                  <div
                    className="bg-purple-500 h-full rounded-full transition-all"
                    style={{ width: `${stats.assignedCompletionRate || 0}%` }}
                  />
                </div>
                <p className="text-[11px] text-text-tertiary font-mono">
                  {stats.completedAssignedPapers || 0} of{" "}
                  {stats.totalAssignedPapers || 0} read ·{" "}
                  {stats.pendingAssignedPapers || 0} pending
                </p>
              </Link>
              ...
              <Link
                href="/papers"
                className="p-4 rounded-2xl glass-card space-y-2 hover:border-success/40 transition-all block group"
              >
                <div className="flex items-center justify-between text-xs">
                  <span className="font-semibold text-text-primary flex items-center gap-1">
                    <TrendingUp size={13} className="text-success" /> Library
                    Read
                  </span>
                ...
                <p className="text-[11px] text-text-tertiary font-mono">
                  {stats.completed} of {stats.totalPapers} cataloged ·{" "}
              ...
              <Link
                href="/labs"
                className="p-4 rounded-2xl glass-card space-y-2 hover:border-accent/40 transition-all block group"
              >
                <div className="flex items-center justify-between text-xs">
                  <span className="font-semibold text-text-primary flex items-center gap-1">
                  ...
                  <span className="font-mono text-accent font-bold">
                    {stats.activeLabTasks || 0} Active
                  </span>
                </div>
                <div className="w-full bg-bg-primary h-2 rounded-full overflow-hidden">
                  <div
                    className="bg-accent h-full rounded-full transition-all"
                    style={{ width: `${stats.activeLabTasks ? 60 : 100}%` }}
                  />
                </div>
                <p className="text-[11px] text-text-tertiary font-mono">
              ...
              <Link
                href="/milestones"
                className="p-4 rounded-2xl glass-card space-y-2 hover:border-amber-500/40 transition-all block group"
              >
                <div className="flex items-center justify-between text-xs">
                  <span className="font-semibold text-text-primary flex items-center gap-1">
                ...
                <div className="w-full bg-bg-primary h-2 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${stats.totalMilestones ? Math.round(((stats.completedMilestones || 0) / stats.totalMilestones) * 100) : 0}%`,
                    }}
                  />
                </div>
                <p className="text-[11px] text-text-tertiary font-mono">
              ...
              <div className="flex items-center gap-3 shrink-0">
                <div className="text-right text-xs font-mono">
                  <span className="text-accent font-semibold block">
                    {new Date(
                      stats.upcomingMeeting.scheduledAt,
            ...
            <div className="glass-card p-4 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-[11px] font-bold uppercase tracking-wide text-text-secondary flex items-center gap-1.5">
                  <MessageSquare size={13} className="text-accent" /> Recent
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-bold text-text-primary flex items-center gap-2 font-display">
              <Clock size={18} className="text-accent" /> Recently Updated
              Literature
            </h3>
            <Link
              href="/papers"
            ...
            {stats.tagDistribution && stats.tagDistribution.length > 0 ? (
              <div className="flex flex-wrap gap-1.5">
                {stats.tagDistribution.map((t) => (
                  <Link
                    key={t.id}
                    href={`/papers?tag=${encodeURIComponent(t.name)}`}
                  >
                    <Badge
                      variant="default"
                      size="sm"
