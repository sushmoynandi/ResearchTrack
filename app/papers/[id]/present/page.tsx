...
export default function JournalClubPresentationPage() {
  ...
  // Fullscreen Slide Viewer State & PDF Page Navigation State
  const [isSlideViewerFullscreen, setIsSlideViewerFullscreen] = useState(false)
  const [pdfPageNumber, setPdfPageNumber] = useState(1)
  const slideViewerRef = useRef<HTMLDivElement>(null)

  const toggleSlideViewerFullscreen = () => {
    if (!document.fullscreenElement) {
      slideViewerRef.current?.requestFullscreen().catch(() => {})
      setIsSlideViewerFullscreen(true)
    } else {
      document.exitFullscreen().catch(() => {})
      setIsSlideViewerFullscreen(false)
    }
  }
