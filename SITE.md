...
## Pages & Routes
- **Landing page** (`/`, signed out) — The public front page anyone sees before they sign in: what ResearchTrack is, what it does, who it's for, and how it keeps work private. **Log in** and **Register** buttons sit in the top bar, in the opening section, and again at the bottom.
- **Dashboard** (`/`, signed in) — Reading pipeline overview, live counters, recent papers, and collection shortcuts
...
## Database
- Powered by Prisma ORM with models: `User`, `Paper`, `Tag`, `Collection`, `Note`, `RoleChangeRequest`, `PasswordResetOtp`, plus lab/collaboration models (29 tables total). Two-factor lives on the `User` row (`twoFactorEnabled`, `twoFactorMethod`, `twoFactorSecret`, `twoFactorEmailReady`).
